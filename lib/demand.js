...
// accepted as נאלצה). One explicit list, not a morphological analyzer; it
// lives in public/tokenize.js so the reader's tinting extends the same list.
const { PROCLITICS } = tok;
...
async function next({ after } = {}) {
  ...
  const reason = verbs === 0
    ? 'None of your verbs is shaky or looked up yet; open a few verb cards in the reader first.'
    : after && verbs === 1
      ? 'That is the only verb with a sentence on record; open more verb cards in the reader.'
      : 'None of your verbs has a sentence on record in a stored article; open verb cards from the reader.';
...
// A card cached before cards carried nikud: the page adds it, with the
// sentence the card was cached under.
const pointsFor = (answer, found) => (answer.points_missing ? { points_missing: true, sentence: found.sentence } : {});
...
async function check(body) {
  ...
  return { ok, surface, card: answer.card, spot: answer.spot, spine: answer.spine, ...pointsFor(answer, found) };
}
...
async function show(body) {
  ...
  return { surface, card: answer.card, spot: answer.spot, spine: answer.spine, ...pointsFor(answer, found) };
}
