...
// spot must have a touch in an article whose stored text still holds that
// surface as a word. No such spot -> { state: 'no data', reason }.
...
async function next({ after } = {}) {
  ...
  for (const spot of candidates) {
    for (const t of db.articleTouches(spot.id)) {
      ...
      const found = findSurface(article.text, t.surface);
      if (!found) continue;
      ...
      return {
        ...
        also: alsoInArticle(article.text, t.surface),
      };
    }
  }
  ...
  const reason = verbs === 0
    ...
    : after && verbs === 1
      ...
      : 'No verb on your map has a sentence on record in a stored article.';
...
function checkFields(body, { needTyped }) {
  ...
  const found = findSurface(article.text, surface);
