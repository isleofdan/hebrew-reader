...

{
  ...
  "sections": [ the study guide, in this order, each section an object with "kind":
    ...
    { "kind": "grammar", "topic": "<topic name>", "guys_lines": [ "<every line of the lesson this section explains, copied exactly as given>" ], "explanation": "<explanation with examples from the lesson>", "examples": [ { "he": "<Hebrew from the lesson>", "en": "<English>" } ], "verb_claims": [ { "verb": "<infinitive>", "takes_object": true | false | "both" } — one for every verb whose object the explanation speaks about ], "for_guy": "<the non-technical explanation suitable for sharing with Guy>" }  — one per grammar topic, in Guy's own order,
...
// finished guide's claims and corrects them. It may correct or remove; it may
// not add (the server compares the guide item for item and refuses additions).
const REVIEW_FRAME = `
=== THE REVIEW ===
...
- Roots: check every root letter by letter against the word, including final forms and weak letters (הסלמה is ס.ל.ם, the root of סולם, not ש.ל.ם).
- Binyanim: check every binyan named (לזנק is Pi'el, not Pa'al).
- Etymologies: remove any you cannot vouch for; never replace one guess with another.
...
- Vowel points and the ⚠️ flags: correct any that are wrong.

Correct what is wrong. Remove what is wrong and cannot be corrected. Do NOT add anything: no new vocabulary rows, expressions, cards, grammar sections, drill verbs, prompts, questions or topics. Keep every field in the same JSON shape.

Answer with ONE JSON object and nothing else: {"guide": <the whole guide, corrected, in the same shape>, "changes": ["<one line per correction, in English: what was wrong, and what it is now>"]}. With nothing to correct, return the guide unchanged and "changes": [].
`;
