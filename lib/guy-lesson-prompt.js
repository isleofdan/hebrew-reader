...

{
  ...
  "sections": [ the study guide, in this order, each section an object with "kind":
    ...
    { "kind": "drills", "verbs": [ 1 or 2 verbs chosen per the Difficulty Register protocol:
        { "verb": "<infinitive>", "root": "א.ב.ג", "binyan": "<binyan>", "deviation": "pe-nun" | "pe-yod" | "guttural" | "hollow" | "doubled" | "none" (or several joined with ", " when the root has more than one, e.g. "pe-nun, doubled"), "takes_object": true | false | "both", "why": "<which Register categories it stacks, and why it was chosen>",
...
- The word cards: the user message also lists the cards already saved from the lesson's single words (each word's lemma, part of speech, root and binyan). Check each card's root and binyan the same way; a wrong card teaches the wrong form every time Dan taps the word.
- The verb cards: for EVERY card in that list whose part of speech is "verb", give your own verdict in "verb_cards", judged from the word itself, not from what the guide says of it (the guide may be wrong).

Do NOT write the guide back. Answer with ONE JSON object and nothing else:
{
  "corrections": [ {
    "section": "grammar" | "drills" | "paper" | "vocabulary" | "questions" | "expressions" | "cards" | "word_cards",
    ...
    "field": "<empty, except as said below>",
  ...
  "verb_cards": [ { "word": "<the verb card's word exactly as the card list gives it>", "verdict": "correct" | "fix", "root": "<the right root as dotted letters, e.g. ז.נ.ק>", "binyan": "<the right binyan: paal, nifal, piel, pual, hifil, hufal or hitpael>", "why": "<empty when correct; else one line in English: what was wrong>" } ]
}
"verb_cards" has one entry for every verb card in the card list, none left out: "correct" when the card's root and binyan are right, "fix" with the right root and binyan when either is wrong. A verb card is corrected by its verdict, never through "word_cards".
To correct any other word card, use section "word_cards", the word exactly as the card list gives it as item, "field": "root" or "binyan", "find": the card's current value, and "replace": the correct value (a root as dotted letters, e.g. "ז.נ.ק"; a binyan as one of paal, nifal, piel, pual, hifil, hufal, hitpael). When the same root or binyan is wrong in the guide too, correct it there as well.
Write "find" from the guide's visible text, the words a reader sees in it, never from its JSON keys or the JSON around a value: never "\"deviation\":\"none\"" or "\"takes_object\":true".
To change a drill verb's deviation, or whether it takes an object, use section "drills", the verb as item, "field": "deviation" or "takes_object", "find": its current value, and "replace": the new value (a deviation from pe-nun, pe-yod, guttural, hollow, doubled, none, several joined with ", " when the root has more than one; takes_object "true", "false" or "both").
To fill a field that is empty (a vocabulary row's "flags" or "pointed", a card's notes), give "field": "flags" | "pointed" | "notes" and "find": "". Otherwise "field" is "" (except on a word card, where it is "root" or "binyan", and on a drill's deviation or takes_object).
Correct only what is wrong. "No change needed" is not a correction; leave out any correction you are not sure of. Never add an item. With nothing to correct, answer {"corrections": [], "remove": [], "verb_cards": [...]}, still with a verdict on every verb card.
`;
...
// `unmet` names what the server's checks still find wrong, so the review fixes
// what a correction can (a missing flag, missing vowel points, an object claim).
// `wordCards` are the lesson's cached word cards ([{ word, lemma, pos, root,
// binyan }]); on a lesson's first build there are none yet.
function reviewMessage({ title, lessonDate, items, guide, unmet = '', wordCards = [] }) {
  const { title: t, date, sections, cards } = guide;
  const told = unmet ? `\n\nThe server's checks still find these problems; fix each one a correction can fix: ${unmet}` : '';
  const words = wordCards.length
    ? `\n\nWord cards saved from this lesson's single words (${wordCards.length}):\n${JSON.stringify(wordCards)}`
    : '\n\nWord cards saved from this lesson\'s single words: none yet.';
  return `Review: ${title}\nLesson date: ${lessonDate || 'unknown'}\nLesson items (${items.length}), one per line:\n${items.join('\n')}${told}${words}\n\nGuide:\n${JSON.stringify({ title: t, date, sections, cards })}`;
}

// The verb-card check: a small call after the review, given only the
// lesson's verb cards, so a wrong binyan is not lost among the guide's
// corrections (the June review proposed 59 and never named לזנק, 23 Sep 2026).
function verbCheckSystem() {
  return `You check word cards of Hebrew verbs for an advanced learner. Each card gives the word as it appeared in a lesson, its dictionary form (lemma), its root and its binyan.
For each card, check the root letter by letter against the word, weak letters included (להקיז is נ.ק.ז; עדה is י.ע.ד), and check the binyan from the word's form (לזנק is Pi'el, not Pa'al; היכון is Nif'al).
Answer ONE JSON object and nothing else:
{"cards": [{"word": "<the word exactly as given>", "verdict": "correct" | "fix", "root": "<the right root as dotted letters, e.g. ז.נ.ק>", "binyan": "<the right binyan: paal, nifal, piel, pual, hifil, hufal or hitpael>", "why": "<empty when correct; else one line in English: what was wrong>"}]}
One entry per card given, in the same order, and no others. On "correct", give the card's own root and binyan. Answer "fix" only when you are sure the card is wrong.`;
}

function verbCheckMessage({ title, cards }) {
  return `Verb cards of the lesson ${title} (${cards.length}), one per line:\n${cards.map((c) => JSON.stringify(c)).join('\n')}`;
}
...
module.exports = { system, reviewSystem, reviewMessage, verbCheckSystem, verbCheckMessage, files: FILES_, CARD_FIELDS, MAX_CARDS, MAX_DRILL_VERBS, MAX_PAPER_PROMPTS, userMessage };
