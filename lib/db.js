...
// touched first. Only spots with at least one touch (a sentence needs one).
function demandCandidates() {
  const rows = db.prepare(`SELECT s.*, (SELECT MAX(t.at) FROM touches t WHERE t.spot_id = s.id) AS last_touch_at
                           ...
                           AND s.status IN ('shaky', 'new') AND last_touch_at IS NOT NULL
...
module.exports = {
  open, STATUSES, THIN_CHARS, ReaderError,
  addArticle, getArticle, listArticles, deleteArticle,
  getCard, putCard,
  getSpot, requireSpot, ensureSpot, setSpotStatus, setOnSpine, listSpots, checkStatus,
  addTouch, articleTouches, lastArticleTitle, marksForSurfaces, cardHint, BINYAN_NAMES,
