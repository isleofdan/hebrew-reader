...
function marksForSurfaces() {
  const rows = db.prepare(`SELECT c.surface, c.spot_id, s.status FROM cards c
                           JOIN spots s ON s.id = c.spot_id
                           WHERE c.spot_id IS NOT NULL ORDER BY c.id ASC`).all();
  const map = {};
  for (const r of rows) map[r.surface] = { spot_id: r.spot_id, status: r.status };
  return map;
}
...
module.exports = {
  ...
  addTouch, lastArticleTitle, marksForSurfaces,
};
