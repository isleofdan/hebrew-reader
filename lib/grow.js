...
// Time limits from what each call writes: an answer of up to 900 tokens (about
// 18 s at 50 tokens a second) and five examples of up to 1,500 (about 30 s),
// plus up to three searches of about 5 s each: 33 s and 45 s expected. Both
// stop at 55 s, under the minute after which a quiet request may be dropped
// on its way to the phone, so a slow call says it timed out rather than
// vanishing.
const ASK = { maxTokens: 900, timeoutMs: 55000 };
const FIND = { maxTokens: 1500, timeoutMs: 55000 };
...
Search Hebrew-language sources online — news sites, television or video transcripts, blogs, forums — and give up to five sentences that use the word in one of the forms listed below, alone or after one prefix such as ו, ש, ה, ב, ל (the reader's app accepts only those forms, so a sentence with another conjugation of the word will be left out).
...
async function findExamples(key) {
  ...
  const forms = desk.formsOf(c);
  const info = {};
  const raw = await lookup.chat({
    system: FIND_SYSTEM, user: `The word: ${word}\nForms to look for: ${forms.join(', ')}\n\n${about}`,
