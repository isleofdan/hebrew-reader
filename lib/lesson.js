...
function markdown(sheet) {
  ...
  lines.push('');
  if (!sheet.count) {
    lines.push(`Nothing qualifies: no word was seen ${sheet.min_touches} or more times in these articles.`);
    return lines.join('\n') + '\n';
  }
  lines.push(`Words you've seen ${sheet.min_touches} or more times, Nif'al first.`);
  for (const g of sheet.groups) {
    ...
    for (const i of g.items) {
      ...
      lines.push(`- ${i.status} · ${i.touches === 1 ? 'seen once' : `seen ${i.touches} times`}`);
    }
  }
  return lines.join('\n') + '\n';
}
