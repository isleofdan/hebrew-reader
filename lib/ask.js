...
The reader checks your answer against these references: ${refs.ASK.map((r) => `${r.id} (${r.label}: ${r.hint})`).join('; ')}.
...
async function ask({ question, article_id }) {
  ...
  if (article_id !== undefined && article_id !== null && article_id !== '') {
    ...
    user += `\n\nThe reader is looking at the article "${article.title}". The words in it the reader has looked up or saved (surface: root/lemma, binyan, status):\n${lines.length ? lines.join('\n') : '(none yet)'}`;
  }
  ...
  return { answer, links, references: refs.ASK.map((r) => ({ id: r.id, label: r.label })), state: 'ok', links_state: links.length ? 'ok' : 'no data' };
}
