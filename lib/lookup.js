...
// `model` defaults to the card model. When another model is asked for and
// OpenRouter answers that it has no such model, the same call is made once
// more with the card model; `info`, when given, is filled with the model
// that answered and `fallback: true`.
// Every model call in the app goes through here.
async function chat({ system, user, maxTokens = 600, what = 'Card could not be built', textFallback = false, timeoutMs = TIMEOUT_MS, model = MODEL, info = null }) {
  ...
  if (!res.ok) {
    ...
    if (model !== MODEL && modelMissing(res.status, detail)) {
      console.error(`model ${model} not found at OpenRouter (${detail || res.status}); retrying once with ${MODEL}`);
      if (info) info.fallback = true;
      return chat({ system, user, maxTokens, what, textFallback, timeoutMs, model: MODEL, info });
    }
...
module.exports = { lookup, cardOnly, chat, contextHash, normalizeRoot, spotFor, shape, modelMissing, MODEL, GUIDE_MODEL, BINYANIM };
