...
async function refreshCard({ surface, sentence }) {
  surface = String(surface || '').trim();
  sentence = String(sentence || '').trim();
  const key = contextHash(surface, sentence);
  const hit = surface ? db.getCard(key) : null;
...
module.exports = { CARD_SYSTEM: SYSTEM, lookup, cardOnly, addPoints, refreshCard, refreshDue, pointedAs, chat, firstJsonObject, schemaRefused, contextHash, normalizeRoot, spotFor, shape, modelMissing, MODEL, GUIDE_MODEL, BINYANIM, BINYANIM_ALL };
