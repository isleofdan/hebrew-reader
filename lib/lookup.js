...
// the cached card, and never again for that card (a form the model gets wrong
// is saved as null, not asked for twice). No bulk backfill. The lookup answers
...
const POINTS_SCHEMA = {
  name: 'pointed_forms',
  schema: { type: 'object', additionalProperties: false, required: ['lemma_pointed', 'surface_pointed'],
    properties: { lemma_pointed: { type: 'string' }, surface_pointed: { type: 'string' } } },
};
...
async function addPoints({ surface, sentence }) {
  ...
  if (hit.card.pointed) return { pointed: hit.card.pointed, called: false };
  ...
  const run = (async () => {
    ...
    const raw = await chat({
      ...
      maxTokens: 200, what: 'Nikud could not be added', schema: POINTS_SCHEMA,
    });
...
async function lookup({ surface, sentence, article_id, lesson_id }) {
  ...
  return { card, spot, cached: fromCache, spine: recorded, ...(card.pointed ? {} : { points_missing: true }) };
}
