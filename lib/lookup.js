...
// The seven, plus the doubled patterns of hollow and doubled roots (Polel,
// Polal, Hitpolel; שוטט is Polel): a card may hold these when Dan sets them or
// a correction gives them. The card call itself still answers the seven.
const BINYANIM_ALL = [...BINYANIM, 'polel', 'polal', 'hitpolel'];
...
A verbal noun (שם פעולה: the action as a noun, e.g. שוטטות, הליכה, כתיבה, התמודדות) is a "noun", never a "verb": its binyan and tense are null, its root is given as for any noun. Only a form of the verb itself (a past, present, future, imperative or infinitive form) is a "verb".
...
async function post({ key, model, messages, maxTokens, format, timeoutMs, what, tools = null }) {
  ...
  try {
    const res = await fetch(OPENROUTER_URL, {
      ...
      body: JSON.stringify({
        ...
        // a call with tools (the web search) asks for no response format:
        // the JSON is read from the answer's text, as for any other answer
        ...(format ? { response_format: format } : {}),
        ...(tools ? { tools } : {}),
        // with a schema, only a provider that honors it may answer
        ...(format && format.type === 'json_schema' ? { provider: { require_parameters: true } } : {}),
        messages,
      }),
    });
...
function contentOf(text, what) {
  try {
    ...
    // the links a web search leaned on come back as url_citation annotations
    const annotations = Array.isArray(choice.message.annotations) ? choice.message.annotations : [];
    const citations = annotations.filter((a) => a && a.type === 'url_citation' && a.url_citation && a.url_citation.url)
      .map((a) => ({ url: String(a.url_citation.url), title: String(a.url_citation.title || ''), content: String(a.url_citation.content || '') }));
    const searches = body.usage && body.usage.server_tool_use ? Number(body.usage.server_tool_use.web_search_requests) || 0 : 0;
    return { content: String(choice.message.content || ''), finish: choice.finish_reason || choice.native_finish_reason || null, citations, searches };
  } catch {
...
async function chat({ system, user, maxTokens = 600, what = 'Card could not be built', textFallback = false, timeoutMs = TIMEOUT_MS, model = MODEL, info = null, schema = null, retryParse = false, tools = null }) {
  if (info) { info.model = model; info.fallback = info.fallback || false; }
  if (tools) return chatWithTools({ system, user, maxTokens, what, textFallback, timeoutMs, model, info, tools });
...
// A call carrying tools (OpenRouter's web search server tool): no response
// format is asked for; the JSON is taken from the answer's text, and `info`
// is given the links the search leaned on (`citations`) and how many
// searches ran (`searches`).
async function chatWithTools({ system, user, maxTokens, what, textFallback, timeoutMs, model, info, tools }) {
  const key = process.env.OPENROUTER_API_KEY;
  if (!key) throw new db.ReaderError(503, `${what}: OPENROUTER_API_KEY is unset.`);
  const messages = [{ role: 'system', content: system }, { role: 'user', content: user }];
  const r = await post({ key, model, messages, maxTokens, format: null, timeoutMs, what, tools });
  if (!r.ok) {
    const detail = errorDetail(r.text);
    throw new db.ReaderError(502, `${what}: OpenRouter answered ${r.status}${detail ? ' (' + detail + ')' : ''}.`);
  }
  const { content, finish, citations, searches } = contentOf(r.text, what);
  if (info) { info.citations = citations; info.searches = searches; info.json = 'none'; }
  const got = firstJsonObject(content);
  if (got.value) return got.value;
  const plain = content.trim();
  if (textFallback && plain) return { text: plain };
  ...
  logRaw(what, model, finish, content, 'first answer');
  throw Object.assign(new db.ReaderError(502, `${what}: the model did not return JSON${got.truncated || finish === 'length' ? ` (its answer was cut off${finish === 'length' ? ` at the ${maxTokens}-token limit` : ''})` : ''}.`), { rawHead: content.slice(0, RAW_HEAD) });
}
...
async function cardOnly({ surface, sentence }) {
  ...
  db.putCard({ surface, contextHash: hash, spotId: spot ? spot.id : null, card, sentence });
...
// The cached card named by its word and sentence, or by its id (the desk,
// which holds cards by id). Answers { key, sentence, hit }.
function cardAt({ surface, sentence, card_id }) {
  if (card_id !== undefined && card_id !== null) {
    const hit = db.getCardById(Number(card_id));
    if (!hit) throw new db.ReaderError(404, `No saved card with id ${card_id}.`);
    return { key: hit.context_hash, sentence: hit.sentence || '', hit };
  }
  surface = String(surface || '').trim();
  sentence = String(sentence || '').trim();
  ...
  return { key, sentence, hit };
}

async function addPoints(which) {
  const { key, sentence, hit } = cardAt(which);
...
// A card whose root or binyan was corrected (by the lesson review, the
// verb-card check, or put back) keeps the pointed forms and the note of the
// card call, written for the old binyan: לזנק showed זָנַק and "in paal"
// under a Pi'el label (live, 23 Sep 2026). After each correction one small
// call re-points the headword and the word as it is in the text and writes
// the note again for the corrected root and binyan, on the nikud fill's JSON
// mode, and saves them. The headword keeps its letters: a pointed form of
// other letters is refused, as the nikud fill refuses it. `refreshed` on the
// card records how many corrections it was refreshed for, so a card opens
// with no call once it is; a failed call is kept on the card with its reason,
// shown there, and tried again at the next open. Answers { card, called,
// failed }.
const REFRESH_SYSTEM = `A Hebrew dictionary card for an advanced reader has had its root or binyan corrected. Its pointed forms and its note were written for the old analysis and must be written again for the corrected one.
Given the word as it appears (the surface), the sentence it came from, its dictionary form (the lemma), the corrected root and binyan, and its meaning, answer with ONE JSON object and nothing else:
{ "lemma_pointed": the dictionary form with full nikud as it is in the corrected binyan (vowel points, dagesh, the shin/sin dot), or "" when no lemma is given,
  "surface_pointed": the word exactly as given, prefixes included, with full nikud as it is read in this sentence in the corrected binyan,
  "note": one short sentence of what is worth noticing about this form in the corrected binyan, or "" }
Never name the old binyan as the word's binyan.`;
const refreshing = new Map(); // card hash -> the call under way

const refreshDue = (card) => Array.isArray(card.corrected) && card.corrected.length > 0
  && !(card.refreshed && !card.refreshed.failed && card.refreshed.for >= card.corrected.length);

async function refreshCard(which) {
  const { key, sentence, hit } = cardAt(which);
  if (!refreshDue(hit.card)) return { card: hit.card, called: false, failed: null };
  if (refreshing.has(key)) return refreshing.get(key);
  const run = (async () => {
    const c = hit.card;
    const n = c.corrected.length;
    let raw, failed = null;
    try {
      raw = await chat({
        system: REFRESH_SYSTEM,
        user: `Corrected card\nSurface: ${c.surface}\nLemma: ${c.lemma || '(none)'}\nRoot: ${c.root || '(none)'}\nBinyan: ${c.binyan || '(not a verb)'}\nMeaning: ${c.meaning_en || ''}\nSentence: ${sentence || '(none given)'}`,
        // two pointed words and a sentence of note: about 120 tokens
        maxTokens: 400, what: 'Nikud and note could not be refreshed', retryParse: true,
      });
      if (raw.error) failed = `the model declined — ${String(raw.error).trim()}`;
    } catch (e) {
      failed = e.message.replace(/^Nikud and note could not be refreshed: /, '');
    }
    const pointed = failed ? null : pointedPair(raw, c.lemma, c.surface);
    if (pointed && !pointed.lemma && !pointed.surface) failed = 'the pointed forms given were not this word';
    const at = new Date().toISOString();
    // read again: a correction may have come in meanwhile
    const now = db.getCard(key);
    const card = failed
      ? { ...now.card, refreshed: { for: n, at, failed } }
      : { ...now.card, pointed, note: typeof raw.note === 'string' && raw.note.trim() ? raw.note.trim() : null, refreshed: { for: n, at } };
    db.putCard({ surface: now.surface, contextHash: key, spotId: now.spot_id, card });
    if (failed) console.error(`lookup: nikud and note not refreshed for the corrected card "${c.surface}": ${failed}`);
    else console.log(`lookup: nikud and note refreshed for the corrected card "${c.surface}" (${c.binyan || c.root}: ${pointed.lemma || 'no headword'}, ${pointed.surface || 'no surface'})`);
    return { card, called: true, failed };
...
async function lookup({ surface, sentence, article_id, lesson_id }) {
  ...
  if (hit) {
    card = hit.card;
    fromCache = true;
    if (hit.sentence === null) db.putCard({ surface: hit.surface, contextHash: hash, spotId: hit.spot_id, card, sentence });
  ...
  // a corrected card not yet refreshed is refreshed by the page's next call,
  // which re-points it too
  return { card, spot, cached: fromCache, spine: recorded, ...(refreshDue(card) ? { refresh_due: true } : hasPoints(card) ? {} : { points_missing: true }) };
}

module.exports = { CARD_SYSTEM: SYSTEM, lookup, hasPoints, askModel, cardOnly, addPoints, refreshCard, refreshDue, pointedAs, chat, firstJsonObject, schemaRefused, contextHash, normalizeRoot, spotFor, shape, modelMissing, MODEL, GUIDE_MODEL, BINYANIM, BINYANIM_ALL };
