...
// or does not answer JSON. Every model call in the app goes through here.
async function chat({ system, user, maxTokens = 600, what = 'Card could not be built' }) {
  ...
  if (!raw) throw new db.ReaderError(502, `${what}: the model did not return JSON.`);
  return raw;
}
