...
async function askModel(surface, sentence) {
  const key = process.env.OPENROUTER_API_KEY;
  if (!key) throw new db.ReaderError(503, 'Card lookup is not configured: OPENROUTER_API_KEY is unset.');
  ...
  try {
    ...
    throw new db.ReaderError(502, `Card could not be built: OpenRouter ${e.name === 'AbortError' ? 'timed out' : 'is unreachable (' + e.message + ')'}.`);
  } finally {
    clearTimeout(timer);
  }
  ...
  if (!res.ok) {
    ...
    throw new db.ReaderError(502, `Card could not be built: OpenRouter answered ${res.status}${detail ? ' (' + detail + ')' : ''}.`);
  }
  let content;
  try { content = JSON.parse(text).choices[0].message.content; } catch {
    throw new db.ReaderError(502, 'Card could not be built: OpenRouter returned an unexpected shape.');
  }
  const raw = parseJsonLoose(content);
  if (!raw) throw new db.ReaderError(502, 'Card could not be built: the model did not return JSON.');
  return raw;
}

// The whole lookup: cache, else model; then spot + touch. Answers
// { card, spot, cached }.
async function lookup({ surface, sentence, article_id }) {
  ...
  if (spotDef) {
    ...
    db.addTouch({ spotId: spotDef.id, articleId: article_id || null, surface });
    spot = db.getSpot(spotDef.id);
  }
  return { card, spot, cached: fromCache };
}

module.exports = { lookup, contextHash, normalizeRoot, spotFor, shape, MODEL, BINYANIM };
