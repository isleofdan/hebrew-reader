...
async function reviewGuide(guide, lesson, info, unmet = []) {
  ...
  if (broken.length) {
    return { guide, review: { state: 'discarded', reason: `The corrected guide failed ${checkNames(broken)}, which the guide had passed — ${broken.map((f) => f.detail).join('; ')}; the guide was kept as built.`, changes, refused, at }, unmet };
  }
  return { guide: reviewed, review: { state: 'applied', changes, refused, card_fixes: cardFixes, at, json: asked.json, ...(asked.retried ? { asked_twice: true } : {}) }, unmet: failures };
}
