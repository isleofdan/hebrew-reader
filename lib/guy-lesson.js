...
async function reviewGuide(guide, lesson, info) {
  ...
  if (!Array.isArray(raw.corrections)) return { guide, review: { state: 'not run', reason: `${REVIEW_WHAT}: the model returned no list of corrections.`, changes: [], refused: [], at } };
  const { guide: reviewed, changes, refused } = applyCorrections(guide, raw);
  const failures = checks.check(reviewed, lesson, knownVerbs(lesson, reviewed));
  if (failures.length) {
    return { guide, review: { state: 'discarded', reason: `The corrected guide failed ${checkNames(failures)} — ${failures.map((f) => f.detail).join('; ')}; the checked guide was kept.`, changes, refused, at } };
  }
...
module.exports = { applyCorrections, extract, nameAndDate, add, shapeGuide, startGuide, view, remove, saveWords, retryWord, plain, MIN_ITEMS, MAX_BYTES, NAME_PATTERN, KIND_ORDER };
