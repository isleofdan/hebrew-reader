...
async function reviewGuide(guide, lesson, info) {
  ...
  try {
    raw = await lookup.chat({
      ...
      maxTokens: GUIDE_MAX_TOKENS, timeoutMs: GUIDE_TIMEOUT_MS, what: REVIEW_WHAT,
      model: info.fallback ? lookup.MODEL : lookup.GUIDE_MODEL, info,
    });
  } catch (e) {
...
module.exports = { extract, nameAndDate, add, shapeGuide, startGuide, view, remove, saveWords, plain, MIN_ITEMS, MAX_BYTES, NAME_PATTERN, KIND_ORDER };
