...
function applyCorrections(checked, raw, breaks = () => []) {
  ...
  for (const { c, run } of steps) {
    if (str(c.section) === 'word_cards') {
      ...
      if (str(c.find) && str(c.find) === c.replace.trim()) continue; // no change: skipped, not listed
  ...
  return { guide, changes, refused, dropped, cardFixes };
}
...
async function correctCards(lesson, fixes, source = 'the lesson review') {
  ...
  for (const f of fixes) {
    ...
    console.log(`lesson ${lesson.id}: card ${surface} ${f.field} ${before} -> ${after} (${source})`
...
async function reviewGuide(guide, lesson, info, unmet = []) {
  ...
  const { guide: reviewed, changes, refused, dropped, cardFixes } = applyCorrections(guide, raw, breaks);
  ...
  return { guide: reviewed, review: { state: 'applied', changes, refused, dropped, card_fixes: cardFixes, at, json: asked.json, ...(asked.retried ? { asked_twice: true } : {}) }, unmet: current };
}
...
// Sep 2026). Fixes go through correctCards, the review's own path: logged,
// the card carrying before and after, the spot following. An answer naming a
// word that is not one of the verb cards sent is refused and listed. Answers
// what goes on the guide as verb_check; never throws.
async function checkVerbCards(lesson, fallback) {
  ...
  const out = { state: 'checked', checked: verbs.length, corrected: [], unchanged: [], refused: [], at };
  ...
  for (const a of list(raw.cards)) {
    ...
    if (str(a.verdict) !== 'fix') continue;
    const root = lookup.normalizeRoot(str(a.root));
    const binyan = checks.binyanKey(a.binyan);
    if (str(a.root) && root !== card.root) fixes.push({ word: card.word, field: 'root', value: str(a.root), why: str(a.why), from: 'card' });
    if (str(a.binyan) && binyan !== card.binyan) fixes.push({ word: card.word, field: 'binyan', value: str(a.binyan), why: str(a.why), from: 'card' });
  }
  const done = await correctCards(lesson, fixes, 'the verb-card check');
  console.log(`lesson ${lesson.id}: verb cards checked in ${Date.now() - t0} ms: ${verbs.length}, corrected ${done.corrected.length}${out.refused.length ? `, answers refused: ${out.refused.join('; ')}` : ''}`);
  return { ...out, corrected: done.corrected, unchanged: done.unchanged, refused: [...out.refused, ...done.refused] };
}
...
async function buildGuide(lesson) {
  ...
  // the verb cards as the review left them, checked on their own
  let verbCheck;
  try { verbCheck = await checkVerbCards(db.getLesson(lesson.id), info.fallback); } catch (e) {
    ...
    verbCheck = { state: 'not run', reason: `${VERB_CHECK_WHAT}: ${e.message}`, checked: 0, corrected: [], unchanged: [], refused: [], at: new Date().toISOString() };
  }
