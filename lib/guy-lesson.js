...
async function reviewGuide(guide, lesson, info, unmet = []) {
  ...
  try {
    raw = await lookup.chat({
      ...
      user: prompt.reviewMessage({ title: lesson.title, lessonDate: lesson.lesson_date, items: lesson.items, guide, unmet: checks.describe(unmet) }),
  ...
  return { guide: reviewed, review: { state: 'applied', changes, refused, at }, unmet: failures };
}
