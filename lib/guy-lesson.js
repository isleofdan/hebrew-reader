...
// the database, so the next open tries again; nothing partial is saved.
...
// For checks: resolves when the lesson's build (if any) is done.
function settled(id) { return building.get(Number(id)) || Promise.resolve(); }

module.exports = { extract, nameAndDate, add, shapeGuide, startGuide, view, settled, plain, MIN_ITEMS, MAX_BYTES, NAME_PATTERN, KIND_ORDER };
