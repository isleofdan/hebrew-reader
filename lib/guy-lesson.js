...
// "Daniel Guy 10FEB2025", "Daniel Guy 21oct2024", and with a word before the
// date, "Daniel Guy hebrew 08july25" (the lesson import, 24 Sep 2026).
const NAME_PATTERN = /^Daniel\s+(?:HEB|Hebrew|Guy)\s+(?:[A-Za-z]+\s+)?(\d{1,2})\s*([A-Za-z]{3})[A-Za-z]*\s*(\d{4}|\d{2})$/i;
...
// and a verdict line per verb card, as the verb-card check writes one: about
// 70 tokens, twice that allowed; three seconds a verb at the slowest
const REVIEW_TOKENS_PER_VERB = 150;
const reviewLimits = (verbs) => ({ maxTokens: REVIEW_MAX_TOKENS + REVIEW_TOKENS_PER_VERB * verbs, timeoutMs: REVIEW_TIMEOUT_MS + 3000 * verbs });
...
const REVIEW_SCHEMA = {
  ...
  schema: {
    ...
    required: ['corrections', 'remove', 'verb_cards'],
    properties: {
      ...
      // the review's own word on every verb card it was given (session ten)
      verb_cards: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['word', 'verdict', 'root', 'binyan', 'why'], properties: {
...
// A drill's fields a correction sets by name, not as text to find.
const DRILL_FIELDS = ['deviation', 'takes_object'];
...
function applyCorrections(checked, raw, breaks = () => []) {
  ...
  const one = (g, c) => {
    ...
    // a drill's deviation or takes_object named by field: set on the data,
    // never edited as text (Personal Shipyard chat, 23 Sep 2026: the review
    // wrote "deviation":"none" as find text, and those fixes were lost)
    const byField = str(c.field);
    if (section === 'drills' && DRILL_FIELDS.includes(byField)) {
      if (typeof c.replace !== 'string' || !c.replace.trim()) return { refused: `no new ${byField} given` };
      const verb = at.list[at.i];
      if (byField === 'takes_object') {
        const v = checks.objectValue(c.replace);
        if (v === null) return { refused: 'takes_object must be true, false or "both"' };
        if (find.trim() && checks.objectValue(find) !== verb.takes_object) return { refused: `the drill's takes_object is ${verb.takes_object}, not ${find.trim()}` };
        if (verb.takes_object === v) return { skip: true };
        at.list[at.i] = { ...verb, takes_object: v };
        return { change: str(c.why) || `${str(c.item)}: takes_object set to ${v}`, fixes: [] };
      }
      const v = c.replace.trim().toLowerCase();
      if (find.trim() && find.trim().toLowerCase() !== str(verb.deviation).toLowerCase()) return { refused: `the drill's deviation is ${verb.deviation || 'empty'}, not ${find.trim()}` };
      if (str(verb.deviation).toLowerCase() === v) return { skip: true };
      at.list[at.i] = { ...verb, deviation: v };
      return { change: str(c.why) || `${str(c.item)}: deviation set to ${v}`, fixes: [] };
    }
...
async function correctCards(lesson, fixes, source = 'the lesson review') {
  const out = { corrected: [], unchanged: [], refused: [], kept: [] };
  ...
  for (const f of fixes) {
    ...
    if (f.field === 'binyan') {
      ...
      if (!lookup.BINYANIM_ALL.includes(after)) { refuse(`"${f.value}" is not a binyan`); continue; }
    } else {
    ...
    // a card Dan confirmed is never changed by a check (session ten)
    if (card.confirmed) {
      keepNote(out.kept, surface, /review/.test(f.by || source) ? 'review' : /verb/.test(f.by || source) ? 'check' : null, f.field, after);
      console.log(`lesson ${lesson.id}: card ${surface} ${f.field} not changed to ${after}, Dan confirmed it (${source})`);
      continue;
    }
    ...
    // a put-back marks the correction it undoes, which the card then leaves out
    let prior = card.corrected || [];
    if (f.restore) {
      entry.restored = true;
      const k = prior.map((x, i) => [x, i]).reverse().find(([x]) => x.field === f.field && !x.undone && same(f.field, x.after, before));
      if (k) prior = prior.map((x, i) => (i === k[1] ? { ...x, undone: true } : x));
    }
    const fixed = { ...card, [f.field]: after, corrected: [...prior, entry] };
    const { def, spot, recorded } = await saveCard(hit, fixed);
  ...
  return { ...out, kept: keptOut(out.kept) };
}

// A card saved as changed: its spot moved to the one the card now names,
// with its status and touches, and the spine told when the spot is marked.
// `hit` is updated in place, so a second change builds on this one.
async function saveCard(hit, fixed) {
...
const BINYAN_NAME = { paal: "Pa'al", nifal: "Nif'al", piel: "Pi'el", pual: "Pu'al", hifil: "Hif'il", hufal: "Huf'al", hitpael: "Hitpa'el", polel: 'Polel', polal: 'Polal', hitpolel: 'Hitpolel' };
const showValue = (field, v) => (field === 'binyan' ? BINYAN_NAME[checks.binyanKey(v)] || str(v) : lookup.normalizeRoot(v) || str(v));

// In the agreement rule only, a doubled pattern counts as the binyan it
// doubles (Personal Shipyard chat, 24 Sep 2026): Polel as Pi'el, Polal as
// Pu'al, Hitpolel as Hitpa'el. The review named לשוטט Pi'el and the verb
// check Polel, the same pattern (26 Jan lesson); the card keeps the name it has.
const FAMILY = { polel: 'piel', polal: 'pual', hitpolel: 'hitpael' };
function agreeKey(field, v) {
  const k = valueKey(field, v);
  return field === 'binyan' ? FAMILY[k] || k : k;
}
const sameHere = (field, a, b) => agreeKey(field, a) === agreeKey(field, b);

// A card Dan confirmed that a check proposed to change, noted for the lesson
// page with what each check proposed (session eleven): kept is a list of
// { word, review, check }, each side "Pa'al", "root ה.מ.ר", "Pa'al, root
// ה.מ.ר" or null when that check proposed nothing. `who` is 'review' or
// 'check'; one proposal per field and side.
function keepNote(kept, word, who, field, value) {
  let k = kept.find((x) => x.word === word);
  if (!k) { k = { word, review: null, check: null, _: { review: {}, check: {} } }; kept.push(k); }
  if (!who || value === null || value === undefined || k._[who][field]) return;
  k._[who][field] = showValue(field, value);
  const got = k._[who];
  k[who] = [got.binyan, got.root ? `root ${got.root}` : null].filter(Boolean).join(', ');
}
// the notes as saved on the guide, without their working part
const keptOut = (kept) => kept.map(({ _, ...k }) => k);
...
// both words). Read only by session nine's one-time put-back, which is
// recorded as run live and never runs again; the agreement rule no longer
// reads the guide as anyone's opinion of a card (session ten: the June guide
// gives לזנק as Pa'al). Answers { root: [values], binyan: [values] }.
...
function agree(lesson, guide, verbCheck) {
  ...
  const passThrough = [];
  // a verb card's opinion from the review is its verdict and nothing else
  // (session ten); the review's card fixes speak for the other cards
  const isVerb = (c) => c.hit.card.pos === 'verb';
  for (const f of review.card_fixes || []) {
    const m = cardsNamed(cards, f.word);
    if (m.length !== 1) { passThrough.push(f); continue; }
    if (isVerb(m[0])) continue;
    const s = slot(m[0]);
    if (!s.review[f.field]) s.review[f.field] = { value: f.value, why: f.why, from: f.from, find: f.find };
  }
  for (const f of review.card_says || []) {
    const m = cardsNamed(cards, f.word);
    if (m.length === 1 && !isVerb(m[0])) { const s = slot(m[0]); (s.said[f.field] = s.said[f.field] || []).push(f.value); }
  }
  for (const a of review.verdicts || []) {
    const m = cardsNamed(cards, a.word).filter(isVerb);
    if (m.length !== 1) continue;
    const s = slot(m[0]);
    for (const field of CARD_FIELDS_FIXABLE) {
      if (s.review[field] || s.said[field]) continue;
      // "fix" with a value: that value; "correct", or a fix leaving this
      // field empty: the card's own value
      if (a.verdict === 'fix' && a[field]) s.review[field] = { value: a[field], why: a.why, from: 'verdict' };
      else s.said[field] = m[0].hit.card[field] ? [m[0].hit.card[field]] : [];
    }
  }
  ...
  for (const s of perCard.values()) {
    ...
    for (const field of CARD_FIELDS_FIXABLE) {
      ...
      const rp = r && !sameHere(field, r.value, cur) ? r.value : null; // the review's proposal
      // what the review says: its proposal, else its fix naming the card's own
      // value, else what it said of the card; never what its guide gives (the
      // June guide gives לזנק as Pa'al, session nine)
      const rSays = rp ? [rp] : r ? [r.value] : s.said[field] || [];
      const ch = s.check && s.check[field];
      const vp = ch && !sameHere(field, ch.value, cur) ? ch.value : null; // the check's proposal
      const vSays = vp ? vp : s.check ? cur : null; // null: the check said nothing of this card
      // a card Dan confirmed: whatever either check proposes, nothing changes,
      // and the page lists it (session ten)
      if (card.confirmed) {
        if (rp) keepNote(kept, s.c.surface, 'review', field, rp);
        if (vp) keepNote(kept, s.c.surface, 'check', field, vp);
        if (rp || vp) console.log(`lesson ${lesson.id}: card ${s.c.surface} ${field} not changed, Dan confirmed it (review ${rp ? showValue(field, rp) : 'no change'}, verb check ${vp ? showValue(field, vp) : 'no change'})`);
        continue;
      }
      if (!rp && !vp) {
        // the review's fix naming what the card already says: listed as
        // unchanged, as before (one naming its doubled pattern changes nothing)
        if (r && same(field, r.value, cur)) fixes.push({ word: s.c.surface, field, value: r.value, why: r.why || '', from: 'agreed', by: 'lesson review' });
        continue;
      }
      const rAgrees = !rSays.length || (rSays.length === 1 && vp && sameHere(field, rSays[0], vp));
      let value = null, by = null;
      if (rp && vp) { if (sameHere(field, rp, vp)) { value = vp; by = 'lesson review and verb check'; } }
  ...
  return { fixes, disagreed, kept: keptOut(kept) };
}
...
async function settleCards(id) {
  ...
  const { fixes, disagreed, kept } = agree(lesson, guide, vc);
  const done = await correctCards(lesson, fixes, 'agreed by the lesson review and the verb-card check');
  // the second guard's notes merged in: a card once, each side's proposal kept
  for (const k of done.kept) {
    const had = kept.find((x) => x.word === k.word);
    if (!had) kept.push(k);
    else { had.review = had.review || k.review; had.check = had.check || k.check; }
  }
  const byReview = (x) => /lesson review/.test(x.by);
  const byCheck = (x) => /verb/.test(x.by);
  const now = db.getLesson(id).guide;
  if (!now) return;
  now.review.cards = { corrected: done.corrected.filter(byReview), unchanged: done.unchanged, refused: done.refused, kept };
...
// A one-time step on the data, run at start and recorded so it never runs
// twice (Personal Shipyard chat, 23 Sep 2026): every card fix the verb-card
// check made on its own before session nine that the same rebuild's review
// contradicted is put back, logged, and the card says so. The review
// contradicts a fix when its card fixes, or else its guide, give that word a
// value and none of them is the one the check put on the card (live: נוצץ,
// Pa'al in the review's drill, turned Pi'el by the check). A card changed
// again since is left. Answers { touched, cards } (also recorded), or null
// when the step had already run.
const RESTORE_STEP = 'restore verb-check card fixes the lesson review contradicted (session nine)';
const RESTORED_WHY = "the verb check's change was contradicted by the lesson review";
async function restoreContradicted() {
  const was = db.stepDone(RESTORE_STEP);
  if (was) { console.log(`one-time step already run on ${was.done_at}: ${RESTORE_STEP}`); return null; }
  const cardsDone = [];
  const toRefresh = [];
  for (const { id } of db.listLessons().items) {
    const lesson = db.getLesson(id);
    const guide = lesson.guide;
    const vc = guide && guide.verb_check;
    // only a check made before session nine, which corrected cards on its own
    if (!vc || vc.state !== 'checked' || vc.answers || !(vc.corrected || []).length) continue;
    const cards = lessonCards(lesson);
    const fixes = [];
    for (const x of vc.corrected) {
      const m = cardsNamed(cards, x.surface);
      if (m.length !== 1) continue;
      const { surface, hit } = m[0];
      if (!same(x.field, hit.card[x.field], x.after)) continue; // changed again since
      const explicit = ((guide.review && guide.review.card_fixes) || [])
        .filter((f) => f.field === x.field && cardsNamed(cards, f.word).some((c) => c.hit.id === hit.id)).map((f) => f.value);
      const says = explicit.length ? explicit : guideSays(guide, [surface, hit.card.lemma])[x.field];
      if (!says.length || says.some((v) => same(x.field, v, x.after))) continue;
      fixes.push({ word: surface, field: x.field, value: x.before, why: RESTORED_WHY, from: 'agreed', by: 'restore', restore: true });
    }
    if (!fixes.length) continue;
    const done = await correctCards(lesson, fixes, 'the one-time restore');
    const now = db.getLesson(id).guide;
    now.verb_check = { ...now.verb_check, restored: [...(now.verb_check.restored || []), ...done.corrected] };
    db.setLessonGuide(id, now);
    for (const c of done.corrected) {
      cardsDone.push({ lesson_id: id, lesson_title: lesson.title, surface: c.surface, field: c.field, from: c.before, to: c.after });
      console.log(`one-time step: lesson ${id} (${lesson.title}): card ${c.surface} ${c.field} ${c.before} -> ${c.after}, restored: ${RESTORED_WHY}`);
    }
    toRefresh.push([lesson, done.corrected]);
  }
  const result = { touched: cardsDone.length, cards: cardsDone };
  db.markStep(RESTORE_STEP, result);
  console.log(`one-time step done: ${RESTORE_STEP}: ${cardsDone.length} card${cardsDone.length === 1 ? '' : 's'} restored${cardsDone.length ? ` (${cardsDone.map((c) => c.surface).join(', ')})` : ''}`);
  for (const [lesson, corrected] of toRefresh) await refreshCards(lesson, corrected);
  return result;
}

// A one-time step on the data, run at start after the put-back above and
// recorded so it never runs twice (Personal Shipyard chat, 24 Sep 2026): the
// put-back turned לזנק in the June lesson back to Pa'al because the June
// guide gives it as Pa'al, but Dan confirmed it as Pi'el live in session eight
// (23 Sep 2026). This card only, no general rule: set to Pi'el, root ז.נ.ק,
// marked confirmed by Dan, the wrong "Restored" line taken off the card and
// the lesson page, logged, and refreshed so its nikud and note are Pi'el.
// Answers { touched, cards } (also recorded), or null when already run.
const CONFIRM_STEP = "put לזנק back to Pi'el, confirmed by Dan (session ten)";
const CONFIRMED = { word: 'לזנק', root: 'ז.נ.ק', binyan: 'piel', on: '2026-09-23' };
const isJune = (l) => l.lesson_date === '2026-06-15' || /\b15\s*jun\s*26\b/i.test(l.source_name || '');
async function confirmZinek() {
  const was = db.stepDone(CONFIRM_STEP);
  if (was) { console.log(`one-time step already run on ${was.done_at}: ${CONFIRM_STEP}`); return null; }
  const cardsDone = [];
  const toRefresh = [];
  for (const { id } of db.listLessons().items.filter(isJune)) {
    const lesson = db.getLesson(id);
    const m = cardsNamed(lessonCards(lesson), CONFIRMED.word);
    if (m.length !== 1) { console.log(`one-time step: lesson ${id} (${lesson.title}): ${m.length} cards named ${CONFIRMED.word}, none changed`); continue; }
    const { surface, hit } = m[0];
    const card = hit.card;
    const before = { root: card.root || null, binyan: card.binyan || null };
    // the put-back's "Restored" entry goes; the correction it undid stands again
    let corrected = (card.corrected || []).filter((x) => !(x.restored && x.field === 'binyan'))
      .map((x) => { if (x.undone && x.field === 'binyan' && same('binyan', x.after, CONFIRMED.binyan)) { const { undone, ...rest } = x; return rest; } return x; });
    const shown = corrected.filter((x) => !x.undone);
    for (const field of CARD_FIELDS_FIXABLE) {
      // a change no entry on the card explains gets one, so the card says what happened
      if (!same(field, before[field], CONFIRMED[field]) && !shown.some((x) => x.field === field && same(field, x.after, CONFIRMED[field]))) {
        corrected = [...corrected, { by: 'Dan', lesson_id: id, lesson_title: lesson.title, field, before: before[field], after: CONFIRMED[field], why: 'confirmed by Dan', at: new Date().toISOString() }];
      }
    }
    const { refreshed, ...rest } = card; // refreshed again for Pi'el
    const fixed = { ...rest, root: CONFIRMED.root, binyan: CONFIRMED.binyan, corrected, confirmed: { on: CONFIRMED.on, root: CONFIRMED.root, binyan: CONFIRMED.binyan } };
    const { spot } = await saveCard(hit, fixed);
    const now = db.getLesson(id).guide;
    if (now && now.verb_check && now.verb_check.restored) {
      const restored = now.verb_check.restored.filter((x) => plain(x.surface) !== plain(surface));
      if (restored.length !== now.verb_check.restored.length) db.setLessonGuide(id, { ...now, verb_check: { ...now.verb_check, restored } });
    }
    cardsDone.push({ lesson_id: id, lesson_title: lesson.title, surface, from: before, to: { root: CONFIRMED.root, binyan: CONFIRMED.binyan } });
    console.log(`one-time step: lesson ${id} (${lesson.title}): card ${surface} binyan ${before.binyan} -> ${CONFIRMED.binyan}, root ${before.root} -> ${CONFIRMED.root}, confirmed by Dan ${CONFIRMED.on}`
      + (spot && spot.replaced ? `, spot ${spot.replaced} -> ${spot.id}` : ''));
    toRefresh.push([lesson, [{ surface }]]);
  }
  const result = { touched: cardsDone.length, cards: cardsDone };
  db.markStep(CONFIRM_STEP, result);
  console.log(`one-time step done: ${CONFIRM_STEP}: ${cardsDone.length} card${cardsDone.length === 1 ? '' : 's'} set${cardsDone.length ? ` (${cardsDone.map((c) => c.surface).join(', ')})` : ''}`);
  for (const [lesson, corrected] of toRefresh) await refreshCards(lesson, corrected);
  return result;
}

// --- Dan sets a verb card's root and binyan -----------------------------------

// A root as Dan types it: 3 or 4 Hebrew letters, with or without dots (or
// spaces, dashes), nikud ignored, a final letter kept as written (ה.מ.ר, המר,
// ק.ל.ך). Answers { root } as dotted letters, or { error } saying why not.
function readRoot(v) {
  const t = String(v || '').normalize('NFC').replace(/[\u0591-\u05C7]/g, '').replace(/[.\s\-'"׳״]/g, '');
  if (!t) return { error: 'Type the root in Hebrew letters, for example ה.מ.ר.' };
  if (/[^א-ת]/.test(t)) return { error: `The root takes Hebrew letters only; "${str(v)}" has other characters.` };
  if (t.length < 3 || t.length > 4) return { error: `A root has 3 or 4 letters; "${str(v)}" has ${t.length}.` };
  return { root: t.split('').join('.') };
}

// Today in Tokyo, where Dan is, as YYYY-MM-DD.
const todayTokyo = () => new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Tokyo' }).format(new Date());

// Dan's own root and binyan for a verb card (session eleven): saved as he
// gives them, changed or as they stand, and the card marked confirmed by him,
// so no check changes it again; he can set it again later. A changed field
// gets a "Set by you" line on the card, and the card is refreshed once so its
// nikud and note match; confirming it as it stands calls nothing. The spot
// follows the card and the spine is told, as for any correction. The card is
// the one the lookup cached for this word in this sentence. Answers { card,
// spot, spine, changed }.
async function confirmCard({ surface, sentence, root, binyan, on = todayTokyo() }) {
  surface = String(surface || '').trim();
  sentence = String(sentence || '').trim();
  const key = lookup.contextHash(surface, sentence);
  const hit = surface ? db.getCard(key) : null;
  if (!hit) throw new ReaderError(404, `No saved card for "${surface}" in that sentence: look the word up first.`);
  const card = hit.card;
  if (card.pos !== 'verb') throw new ReaderError(400, `This card is a ${card.pos || 'word'}, not a verb: only a verb's root and binyan are set here.`);
  const b = checks.binyanKey(binyan);
  if (!lookup.BINYANIM_ALL.includes(b)) throw new ReaderError(400, 'Choose a binyan from the list.');
  const r = readRoot(root);
  if (r.error) throw new ReaderError(400, r.error);
  const want = { root: r.root, binyan: b };
  const at = new Date().toISOString();
  const lines = CARD_FIELDS_FIXABLE.filter((f) => !same(f, card[f], want[f]))
    .map((f) => ({ by: 'Dan', field: f, before: card[f] || null, after: want[f], why: 'set by you', at }));
  const fixed = { ...card, root: want.root, binyan: want.binyan, corrected: [...(card.corrected || []), ...lines], confirmed: { on, root: want.root, binyan: want.binyan } };
  const { spot, recorded } = await saveCard(hit, fixed);
  console.log(`card ${surface}: set by Dan, ${lines.length ? lines.map((x) => `${x.field} ${x.before} -> ${x.after}`).join(', ') : `confirmed as it stands (${want.binyan}, ${want.root})`}, confirmed ${on}`
    + (spot && spot.replaced ? `, spot ${spot.replaced} -> ${spot.id}` : '') + (recorded ? `, spine ${recorded}` : ''));
  // nikud and note written again for what changed; never in the way of the save
  if (lines.length) {
    try { await lookup.refreshCard({ surface, sentence }); } catch (e) { console.error(`card ${surface}: not refreshed: ${e.message}`); }
  }
  const now = db.getCard(key);
  return { card: now.card, spot: now.spot_id ? db.getSpot(now.spot_id) : null, spine: recorded, changed: lines.map((x) => x.field) };
}

// A one-time step on the data, run at start and recorded so it never runs
// twice (Personal Shipyard chat, 24 Sep 2026): a lesson whose file name the
// pattern now reads but did not when it was uploaded — so it took the upload
// day and the file name as its title — gets the date and title the pattern
// gives. Live that is one lesson, from "Daniel Guy hebrew 08july25.pdf",
// dated 24 Sep 2026 by the import, now 8 Jul 2025. Its guide stays. Answers
// { touched, lessons } (also recorded), or null when already run.
const REDATE_STEP = 're-date lessons whose file name has a word before the date (session eleven)';
function redateLessons() {
  const was = db.stepDone(REDATE_STEP);
  if (was) { console.log(`one-time step already run on ${was.done_at}: ${REDATE_STEP}`); return null; }
  const done = [];
  for (const l of db.listLessons().items) {
    const named = nameAndDate(l.source_name);
    const base = String(l.source_name || '').replace(/\.pdf$/i, '').trim();
    // uploaded before the pattern read it: its title is the file name
    if (named.from !== 'file name pattern' || l.title !== base) continue;
    db.setLessonDate(l.id, { title: named.title, lesson_date: named.lesson_date });
    done.push({ id: l.id, source_name: l.source_name, from: { title: l.title, lesson_date: l.lesson_date }, to: { title: named.title, lesson_date: named.lesson_date } });
    console.log(`one-time step: lesson ${l.id} (${l.source_name}): date ${l.lesson_date} -> ${named.lesson_date}, title "${l.title}" -> "${named.title}", guide kept`);
  }
  const result = { touched: done.length, lessons: done };
  db.markStep(REDATE_STEP, result);
  console.log(`one-time step done: ${REDATE_STEP}: ${done.length} lesson${done.length === 1 ? '' : 's'} re-dated`);
  return result;
}

// A one-time step on the data, run at start and recorded so it never runs
// twice (Personal Shipyard chat, 24 Sep 2026): the live card שוטטות of the
// 26 Jan 2026 lesson was made before the card maker was told a verbal noun is
// a noun, so it says verb and is sent to the verb check. It is made again by
// the card maker as it is now, in its own sentence; the card keeps its place
// in the cache (a desk that holds it still holds it) and its spot follows it.
// A card Dan confirmed is left as he set it, and so is one already a noun.
// Answers { touched, cards } (also recorded), or null when already run.
const REMAKE_STEP = 'make the 26 Jan 2026 card שוטטות again, as a noun (session twelve)';
const REMAKE_WORD = 'שוטטות';
const isJan26 = (l) => l.lesson_date === '2026-01-26' || /\b26\s*jan\s*(20)?26\b/i.test(l.source_name || '');
async function remakeShotetut() {
  const was = db.stepDone(REMAKE_STEP);
  if (was) { console.log(`one-time step already run on ${was.done_at}: ${REMAKE_STEP}`); return null; }
  const done = [];
  for (const { id } of db.listLessons().items.filter(isJan26)) {
    const lesson = db.getLesson(id);
    for (const { surface, sentence, hit } of cardsNamed(lessonCards(lesson), REMAKE_WORD)) {
      const card = hit.card;
      if (card.confirmed) { console.log(`one-time step: lesson ${id} (${lesson.title}): card ${surface} confirmed by Dan, left as it is`); continue; }
      if (card.pos !== 'verb') { console.log(`one-time step: lesson ${id} (${lesson.title}): card ${surface} is already a ${card.pos}, left as it is`); continue; }
      const made = lookup.shape(await lookup.askModel(hit.surface, sentence), hit.surface);
      const fixed = { ...made, remade: { on: new Date().toISOString(), why: 'made again: a verbal noun is a noun', before: { pos: card.pos, root: card.root, binyan: card.binyan } } };
      const { spot } = await saveCard(hit, fixed);
      done.push({ lesson_id: id, lesson_title: lesson.title, surface, from: { pos: card.pos, binyan: card.binyan }, to: { pos: made.pos, binyan: made.binyan } });
      console.log(`one-time step: lesson ${id} (${lesson.title}): card ${surface} made again: ${card.pos} (${card.binyan}) -> ${made.pos}${made.binyan ? ` (${made.binyan})` : ''}`
        + (spot && spot.replaced ? `, spot ${spot.replaced} -> ${spot.id}` : ''));
    }
  }
  const result = { touched: done.length, cards: done };
  db.markStep(REMAKE_STEP, result);
  console.log(`one-time step done: ${REMAKE_STEP}: ${done.length} card${done.length === 1 ? '' : 's'} made again`);
  return result;
}
...
async function reviewGuide(guide, lesson, info, unmet = []) {
  ...
  let raw;
  const wordCards = wordCardsForReview(lesson);
  try {
    raw = await lookup.chat({
      system: prompt.reviewSystem(),
      user: prompt.reviewMessage({ title: lesson.title, lessonDate: lesson.lesson_date, items: lesson.items, guide, unmet: checks.describe(unmet), wordCards }),
      ...reviewLimits(wordCards.filter((c) => c.pos === 'verb').length), what: REVIEW_WHAT,
  ...
  const vv = verbVerdicts(lesson, raw.verb_cards);
  console.log(`lesson ${lesson.id}: review verdicts on verb cards: ${vv.verdicts.length} of ${vv.sent}${vv.missing.length ? `, none for ${vv.missing.join(', ')}` : ''}${vv.refused.length ? `, refused: ${vv.refused.join('; ')}` : ''}`);
  return { guide: reviewed, review: { state: 'applied', changes, refused, dropped, card_fixes: cardFixes, card_says: cardSays, verdicts: vv.verdicts, verdicts_missing: vv.missing, verdicts_refused: vv.refused, at, json: asked.json, ...(asked.retried ? { asked_twice: true } : {}) }, unmet: current };
}

// The review's verdict on each verb card it was given (session ten): the
// card's word, "correct" or "fix", and the root and binyan it gives. An entry
// finds its card by the word, the card's surface or its lemma, nikud ignored,
// among this lesson's verb cards; one naming none of them, or a second one for
// the same card, is refused and listed. A binyan in any spelling the server
// reads ("Pi'el", "piel") is taken; one it cannot read is left out of the
// verdict, and so is a root with no letters. A verb card with no entry is one
// the review said nothing about. Answers { verdicts, missing, refused, sent }.
function verbVerdicts(lesson, given) {
  const verbs = lessonCards(lesson).filter(({ hit }) => hit.card.pos === 'verb');
  const out = { verdicts: [], missing: [], refused: [], sent: verbs.length };
  const seen = new Set();
  for (const a of list(given)) {
    const m = cardsNamed(verbs, a.word);
    if (m.length !== 1) { out.refused.push(`${str(a.word) || '?'}: ${m.length ? `matches ${m.length} verb cards; not guessed` : "not one of this lesson's verb cards"}`); continue; }
    const { surface, hit } = m[0];
    if (seen.has(hit.id)) { out.refused.push(`${surface}: a second verdict on the same card`); continue; }
    seen.add(hit.id);
    const verdict = str(a.verdict).toLowerCase() === 'fix' ? 'fix' : 'correct';
    const b = checks.binyanKey(str(a.binyan));
    const v = { word: surface, verdict, root: lookup.normalizeRoot(str(a.root)) || '', binyan: lookup.BINYANIM_ALL.includes(b) ? b : '', why: str(a.why) };
    // a fix gives both fields; the one it leaves as the card has it is not
    // part of the fix, unless neither differs (a fix naming the card's own
    // value, listed as unchanged)
    const differs = CARD_FIELDS_FIXABLE.filter((f) => v[f] && !same(f, v[f], hit.card[f]));
    if (verdict === 'fix' && differs.length) for (const f of CARD_FIELDS_FIXABLE) if (!differs.includes(f)) v[f] = '';
    out.verdicts.push(v);
  }
  for (const { surface, hit } of verbs) if (!seen.has(hit.id)) out.missing.push(surface);
  return out;
}
...
module.exports = { applyCorrections, correctCards, confirmCard, redateLessons, remakeShotetut, readRoot, sameInAgreement: sameHere, restoreContradicted, confirmZinek, extract, nameAndDate, add, shapeGuide, startGuide, view, remove, saveWords, retryWord, plain, MIN_ITEMS, MAX_BYTES, NAME_PATTERN, KIND_ORDER };
