...
function checkDrill(guide, verbs) {
  ...
  if (!DEVIATIONS.includes(dev)) problems.push(`the first drill (${first.verb}) gives no deviation from ${DEVIATIONS.join(', ')}`);
  ...
  if (card && card.binyan && claimed && card.binyan !== claimed) problems.push(`the drill calls ${first.verb} ${first.binyan}; its card says ${card.binyan}`);
  const root = (card && card.root) || first.root;
  if (IRREGULAR.includes(dev) && !rootDeviations(root).includes(dev)) problems.push(`the drill calls ${first.verb} ${dev}, which its root ${root || '(none given)'} is not`);
  const firstIrregular = claimed === 'nifal' || (IRREGULAR.includes(dev) && rootDeviations(root).includes(dev));
...
module.exports = { check, describe, plain, binyanKey, rootDeviations, irregular, objectValue, RULES, DEVIATIONS, NAMES: Object.keys(RULES) };
