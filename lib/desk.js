...
function put({ surface, sentence, card_id }) {
  ...
  if (card_id !== undefined && card_id !== null) row = db.getCardById(Number(card_id));
    ...
    if (row && row.sentence === null) db.putCard({ surface: row.surface, contextHash: row.context_hash, spotId: row.spot_id, card: row.card, sentence: said });
...
function sentenceHasWord(sentence, c) {
  ...
  for (const w of [k.surface, c.sentence_surface]) if (w) marks[plainOf(w)] = true;
  if (k.lemma) lemmas[plainOf(k.lemma)] = true;
  const row = c.card_id ? db.getCardById(c.card_id) : null;
  if (row && row.spot_id) for (const s of db.surfacesOfSpot(row.spot_id)) marks[plainOf(s)] = true;
...
function find(q, { source = 'everything', time = 'any' } = {}) {
  ...
  for (const c of cards) c.origin_line = originLine(c);
...
module.exports = {
  ...
  branch, cut, thread, threadLine, lastTouched, origin, full, sentenceHasWord, day, pastDesk, rootOf, SOURCES, TIMES,
};
