...
function move(deskId, key, { x, y, w, h, front, free } = {}) {
  ...
  if (free && x === undefined && y === undefined) {
    ...
    const parent = db.linksOf([key]).find((l) => l.child === key);
    const near = parent ? db.getPlacement(deskId, parent.parent) : null;
...
function find(q) {
  ...
  for (const d of list().items) {
    ...
    const parents = new Map(db.linksOf(placed).map((l) => [l.child, l.parent]));
    ...
    const why = hit.filter((key) => !grewFromHit(key))
      ...
      .map((key) => ({ card: key, label: label(content(key)), grown: db.descendants(key).filter((k) => placed.includes(k)).length }));
