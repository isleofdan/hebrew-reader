...
function noteLink(id) {
  ...
  const url = n.link_url && String(n.text || '').includes(n.link_url) ? n.link_url : linkIn(n.text);
...
function catchCard({ text, url, title } = {}) {
  ...
  let body = bits.join('\n');
  if (link && !body.includes(link)) body = body ? `${body}\n${link}` : link;
  body = body.trim();
...
function undoStroke(key) {
  ...
  return db.together(() => {
    ...
    if (!left) db.removeInk(id);
    for (const d of db.desksHolding(key)) db.touchDesk(d.id);
    return { layer: left ? content(ink.key) : null, removed };
  });
}
...
module.exports = {
  ...
  linkLabels, inkOf, addStroke, undoStroke, link, linkedOf, groupsOf, sheetsLine, sheetGroups, sheetCard, addSheet, catchCard, setNoteTitle, linkIn,
};
